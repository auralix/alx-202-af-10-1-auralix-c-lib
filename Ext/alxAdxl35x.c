...
typedef union
{
	...
	uint8_t raw[12];
} AlxAdxl35x_Xyz_20bit;

typedef union
{
	uint16_t val;
	uint8_t raw[2];
} AlxAdxl35x_Temp_12bit;


//******************************************************************************
// Private Functions
//******************************************************************************
static void AlxAdxl35x_RegStruct_SetAddr(AlxAdxl35x* me);
...
static Alx_Status AlxAdxl35x_TraceId(AlxAdxl35x* me);
static AccDataPoint AlxAdxl35x_ConvertXyz(AlxAdxl35x* me, AlxAdxl35x_Xyz_20bit xyz_20bit);
...
)
{
	...
	// Variables
	AlxAdxl35x_RegStruct_SetAddr(me);
	AlxAdxl35x_RegStruct_SetLen(me);
	AlxAdxl35x_RegStruct_SetValToZero(me);
...
)
{
	...
	ALX_ADXL35X_ASSERT(me->isInit == false);
	...
	AlxAdxl35x_RegStruct_SetValToDefault(me);
...
Alx_Status AlxAdxl35x_DeInit(AlxAdxl35x* me)
{
	// Assert
	ALX_ADXL35X_ASSERT(me->wasCtorCalled == true);
	ALX_ADXL35X_ASSERT(me->isInit == true);
...
/**
  ...
  * @retval			Alx_Err
  */
static Alx_Status AlxAdxl35x_GetXyz_g(AlxAdxl35x* me, AccDataPoint* data)
{
	// Assert
	ALX_ADXL35X_ASSERT(me->wasCtorCalled == true);
	ALX_ADXL35X_ASSERT(me->isInit == true);
	...
	// Convert and return
	*data = AlxAdxl35x_ConvertXyz(me, xyz_20bit);
	return Alx_Ok;
}
...
/**
  ...
  * @retval			Alx_Err
  */
static Alx_Status AlxAdxl35x_GetFifoXyz_g(AlxAdxl35x* me, AccDataPoint* data, uint8_t len)
{
	// Assert
	ALX_ADXL35X_ASSERT(me->wasCtorCalled == true);
	ALX_ADXL35X_ASSERT(me->isInit == true);
	ALX_ADXL35X_ASSERT((1 <= len) && (len <= 32));
	...
	for (uint32_t i = 0; i < len; i++)
	{
		...
		// Convert
		_xyz_g[i] = AlxAdxl35x_ConvertXyz(me, xyz_20bit);
	}
...
/**
  ...
  * @retval			Alx_Err
  */
static Alx_Status AlxAdxl35x_GetTemp_degC(AlxAdxl35x* me, float* temp_degC)
{
	// Assert
	ALX_ADXL35X_ASSERT(me->wasCtorCalled == true);
	ALX_ADXL35X_ASSERT(me->isInit == true);
	...
	// Convert
	AlxAdxl35x_Temp_12bit temp_12bit = {};
...
/**
  ...
  * @retval			Alx_Err
  */
static Alx_Status AlxAdxl35x_GetStatusReg(AlxAdxl35x* me, AlxAdxl35x_RegVal_0x04_Status* statusReg)
{
	// Assert
	ALX_ADXL35X_ASSERT(me->wasCtorCalled == true);
	ALX_ADXL35X_ASSERT(me->isInit == true);
...
static void AlxAdxl35x_RegStruct_SetVal(AlxAdxl35x* me, float sampleRate, AlxAccSyncMode syncMode)
{
	...
	{
		ALX_ADXL35X_ASSERT(false);	// We should never get here
	}
	...
	{
		me->reg._0x2C_Range.val.Range = me->range;
	}
	else
	{
		ALX_ADXL35X_ASSERT(false);	// We should never get here
...
static AccDataPoint AlxAdxl35x_ConvertXyz(AlxAdxl35x* me, AlxAdxl35x_Xyz_20bit xyz_20bit)
{
	...
	const float rangeFactors_355[Range_Guard] = {0.0f, 0.0000039f, 0.0000078f, 0.0000156f};

...
Alx_Status AlxMemSafe_Read(AlxMemSafe* me, uint8_t* data, uint32_t len)
{
	...
	else
	{
		...
		for (uint32_t i = 0; i < me->memSafeReadWriteNumOfTries; i++)
		{
			...
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnInit");
				continue;
			}
			...
			if(statusMemRaw != Alx_Ok)
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnReadCopyA");
				continue;
			}
			...
			if(statusMemRaw != Alx_Ok)
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnDeInit");
				continue;
			}
			...
			if(statusMemRaw != Alx_Ok)
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnReadCopyB");
				continue;
			}
...
Alx_Status AlxMemSafe_Write(AlxMemSafe* me, uint8_t* data, uint32_t len)
{
	...
	else
	{
		...
		for (uint32_t i = 0; i < me->memSafeReadWriteNumOfTries; i++)
		{
			...
			if(statusMemRaw != Alx_Ok)
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnWriteCopyA");
				continue;
			}
			...
			if(statusMemRaw != Alx_Ok)
			{
				ALX_MEM_SAFE_TRACE_WRN("WrnWriteCopyB");
				continue;
			}
